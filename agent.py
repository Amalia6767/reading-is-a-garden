...
def explain_bite(title, k, total, bite):
    """园丁讲解今天这一口。"""
    context = f"这是论文《{title}》共 {total} 口中的第 {k} 口" \
              + ("(第一口,请先用两三句话介绍这篇论文大概讲什么)" if k == 1
                 else "(前面几口已经讲过,直接接着讲,开头简单一句'上回说到…'衔接即可)")
...
def save_bite(title, k, total, content):
    ...
    section = f"## 🍃 第 {k}/{total} 口 · {today}\n\n{content}\n"
    if k == 1 or not os.path.exists(path):
        body = f"# 🌱 {title}\n\n> 种下于 {today} · 全文共 {total} 口\n\n{section}"
        mode = "w"
    else:
        body = f"\n---\n\n{section}"
        mode = "a"
...
def main():
    ...
    if rec["done"] == rec["total"]:
        ...
        print(f"\n🍃 这株还差 {rec['total'] - rec['done']} 口,明天继续,它会一截一截长高。")
